  ...
  return (
    <div className="flex flex-col h-full justify-between gap-3 select-none" id="cartridge-rules">
      ...
      <div className="flex items-center justify-between pb-2 border-b-2 border-[#2b2e30]">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="bg-[#241818] text-[#f2933d] border border-[#422525] font-silkscreen text-[9.5px] px-2 py-0.5 rounded-xs">
              OFFICIAL PROTOCOL V2.6
            </span>
          </div>
          <p className="font-silkscreen text-[10px] sm:text-[11px] text-[#8f9396]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 grow overflow-hidden">
          ...
          {RULES.map((rule, idx) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-2.5 rounded-md border-2 transition-none cursor-pointer flex items-center justify-between
                           ${
                             isSelected
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="truncate">
                    <span className="font-pixel text-[10.5px] sm:text-[11.5px] text-[#cfe8ff] block truncate">
                      {rule.title}
                    </span>
                    <span className="font-silkscreen text-[9px] sm:text-[9.5px] text-[#8f9396]">
                      {rule.category} &bull; {rule.status}
                    </span>
                  </div>
                </div>
                {isSelected && <span className="text-[#f4c151] font-pixel text-[11px]">&gt;</span>}
              </button>
            );
          })}
        </div>
        ...
        {/* Right Detail Pane */}
        <div className="lg:col-span-7 flex flex-col justify-between p-3.5 rounded-lg bg-[#141618] border-2 border-[#2b2e30] shadow-inner overflow-y-auto">
          <div className="space-y-2.5">
            <div className="flex items-center justify-between border-b border-[#2b2e30] pb-2">
              ...
              <span
                ...
                className="font-silkscreen text-[9.5px] px-2 py-0.5 rounded bg-[#1e2225] border border-[#33373a]"
              >
            ...
            <div className="space-y-1">
              <span className="font-silkscreen text-[10px] text-[#8f9396] uppercase block">
                REQUIREMENT:
              </span>
              <p className="font-silkscreen text-[11px] sm:text-[12px] text-[#d6e2eb] leading-relaxed">
                {RULES[selectedRule].description}
              </p>
            </div>

            <div className="p-2.5 rounded bg-[#1c1414] border border-[#442222] space-y-1">
              <span className="font-silkscreen text-[9px] text-[#ef4444] uppercase tracking-wider flex items-center gap-1">
                <AlertTriangle className="h-3 w-3" /> ENFORCEMENT &amp; PENALTY:
              </span>
              <p className="font-silkscreen text-[10px] sm:text-[10.5px] text-[#e0a2a2] leading-normal">
          ...
          <div className="mt-3 pt-2 border-t border-[#2b2e30]">
            <span className="font-silkscreen text-[10px] text-[#8f9396] uppercase block mb-1.5 flex items-center gap-1">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
              {JUDGING_CRITERIA.map((crit, i) => (
                <div key={i} className="p-1.5 rounded bg-[#1b1f23] border border-[#2d343a] text-center">
                  ...
                  <span className="font-silkscreen text-[9px] text-[#cfe8ff] block truncate">{crit.label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Footer Banner */}
      <div className="pt-2 border-t border-[#26282a] flex items-center justify-between text-[9.5px] font-silkscreen text-[#7d8285]">
